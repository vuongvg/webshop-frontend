...
import { fetchApiGetAllOrder } from "../../../../common/fetchApi";
...
function Order() {
   ...
   useEffect(() => {
      fetchApiGetAllOrder().then((res) => setOrder(res));
   }, []);
               ...
               <tbody>
                  ...
                  {0 ? (
                     new Array(10).fill(null).map((item, index) => (
                        <tr key={index}>
                           <td colSpan={5}>{/* <Skeleton height={24.4} count={1} /> */}</td>
                        </tr>
                     ))
                  ) : order.length === 0 ? (
                     <tr>
                        <td colSpan={5}>
                           <p className="text-center alert alert-warning">You don't have any orders</p>
                        </td>
                     </tr>
                  ) : (
