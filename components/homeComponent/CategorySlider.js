...
function CategorySlider({ dataCategorySlider }) {
   useEffect(() => {
      (function ($) {
         ...
         //    parent.css({
         //       "background-image": "url(" + src + ")",
         //       // "background-image": src.map(img=>"url(" + img + ")").join(','),
         //       "background-size": "cover",
         //       "background-position": "center",
         //       "background-repeat": "no-repeat",
         //       display: "block",
            ...
            <div className="row gy-3">
               ...
               <div className="col-xxl-10 col-lg-9">
                  <div className="category-wrapper category-slider1 white-arrow category-arrow">
                     ...
                     {dataCategorySlider.map((category, index) => (
                        <div key={index}>
                           <Link href={"/product-category/" + category.slug}>
                              <a className="category-wrap category-padding">
                                 {/* <img src={category.acf.thumbnail_home} className="bg-img blur-up lazyload" alt={category.name} /> */}
                                 <img src={category.image.src} className="bg-img lazyload" alt={category.name} />
                                 <div className="category-content category-text-1">
                                    <h3 className="theme-color">{category.name}</h3>
